...
export function Habit() {
  ...
  async function handToggleHabit(habitId: string) {
    try {
  ...
  return (
    <View className="flex-1 bg-background px-8 pt-16">
      ...
      >
        ...
        <Text className='mt-6 text-zinc400 font-semibold text-base lowercase'>
          {dayOfWeek}
        </Text>
        
        <Text className='font-white font-extrabold text-3lx'>
          {dayAndMonth}
        </Text>
        ...
        })}>
          {
            ...
            dayInfo?.possibleHabits.map(habit => (
              <Checkbox
                ...
                onPress={() => {handToggleHabit(habit.id)}}
              />
            ))
            : <HabitsEmpty/>
          }
